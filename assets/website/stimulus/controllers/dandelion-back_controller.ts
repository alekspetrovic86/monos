...
export default class DandelionController extends Controller {
    ...
    private _setupObservers(): void {
        ...
        this._introObserver = new IntersectionObserver(entries => {
            entries.forEach(e => {
                ...
                setTimeout(() => { this._introProgress = 0; }, 300); // 300ms delay then needles grow
                this._introObserver.disconnect();
            });
        }, { rootMargin: '0px', threshold: 0.5 });
        // Observer is attached to the canvas after connect() sets it up — deferred one tick
        setTimeout(() => {
    ...
    private _animate(): void {
        ...
        // Advance intro: full grow in ~45 frames (≈0.75s at 60fps)
        const introActive = this._introProgress >= 0 && this._introProgress < 1;
        if (introActive) {
            this._introProgress = Math.min(1, this._introProgress + 0.022);
        }
        ...
        for (const line of this._lines) {
            ...
            g.addColorStop(0.28, rgba(C.violet, 0));
            g.addColorStop(0.43, rgba(C.violet, 0.18 * fin));
            g.addColorStop(0.55, rgba(C.violet, 0.45 * fin));
            g.addColorStop(1,    rgba(C.indigo,  0.8 * fin));
        ...
        ctx.shadowBlur  = 8;
        ctx.shadowColor = rgba(C.purpleDark, 0.4);
